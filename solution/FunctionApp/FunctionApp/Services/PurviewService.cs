    ...
    public class PurviewService
    {
        ...
        Logging.Logging logging)
            ...
            try
            {
                ...
                {
                    if (!Helpers.JsonHelpers.CheckForJsonProperty(child, metadata))
                        logging.LogErrors(new Exception(metadata + "[" + child + "] not found and is necessary to properly log to Purview"));
                }
                ...
                foreach (var child in children)
                {
                    if (!Helpers.JsonHelpers.CheckForJsonProperty(child, parent))
                        logging.LogErrors(new Exception(parent + "[" + child + "] not found and is necessary to properly log to Purview"));
                }
            ...
            for (int i = 0; i < toIterate.Length; i++)
            {
                ...
                switch (metadata["TaskObject"][choice]["System"]["Type"].ToString())
                {
                    case "Delta":
                        ...
                        QualifiedName = metadata["TaskObject"][choice]["System"]["SystemServer"] + "/" + metadata["TaskObject"][choice]["System"]["Container"] + "/" + modifiedRelativePath + "/" + metadata["TaskObject"][choice]["DataFileName"] + "/{SparkPartitions}";
                        break;
                    default:
                        //datasetName = metadata["TaskObject"][choice]["DataFileName"].ToString().Split(new Char[] { ',', '.' })[0];
                        modifiedRelativePath = Regex.Replace(metadata["TaskObject"][choice]["Instance"][choice + "RelativePath"].ToString(), "[0-9]{2,}", "{N}");
                        QualifiedName = metadata["TaskObject"][choice]["System"]["SystemServer"] + "/" + metadata["TaskObject"][choice]["System"]["Container"] + "/" + modifiedRelativePath + "/" + metadata["TaskObject"][choice]["DataFileName"];
                        break;
                }
