...
{
    local TargetPrefixForActivityName = if (Staging==true) then "Stage" else "Target",
    local TargetPrefixForParameters = if (Staging==true) then "Staging" else "",    
    "source": {
      "type": DataFactorySourceType,
      "sqlReaderQuery": {
          "value": "@activity('AF Get Information Schema SQL "+TargetPrefixForActivityName+"').output.InformationSchemaSQL",
          "type": "Expression"
      },
