...
declare @systemwideconcurrency integer = 19
...
Select c0.*,
	...
	SlotsToBeAssignedPerGroup = case when floor(c1.AdjustedSlotsToBeAssigned/ c0.TaskGroups) = 0 then 1 else floor(c1.AdjustedSlotsToBeAssigned/ c0.TaskGroups) end
into #SystemWideView
from
	(Select 1 as dummy) d
	cross apply
	(
		Select SlotsConsumed = sum(TasksAssigned),
		sum(ConcurrencySlotsAvailableInGroup) AdditionalSlotsAvailable,
		count(TaskGroupId) TaskGroups,
