...
BEGIN
	...
	from TaskGroup TG
	left join #NewAndAssigned NA on NA.TaskGroupId = TG.TaskGroupId
	where (NewTasks > 0 or TasksAssigned > 0) and (TG.ActiveYN = 1)
	...
	from
		...
		(
			...
			isnull(sum(case when TaskCount > 0 then ConcurrencySlotsAvailableInGroup else 0 end),0) AdditionalSlotsAvailable,
			isnull(count(case when TaskCount > 0 then TaskGroupId else null end),0) TaskGroups,
	...
	Select *, NewTIsAssigned = (Select count(*) from #TasksToUpdate_WithRunnerId)
	from #SystemWideView
